...
1. In Coolify, create a new project and add a resource using the **`docker-compose.coolify.yml`** file from the repo (use this file as the compose source).
2. Leave **"Escape special characters in labels"** unchecked so `${DOMAIN_NAME}` in Traefik labels is resolved.
3. Set the required environment variables in Coolify (e.g. `DOMAIN_NAME`, `BASE_URL`, `BETTER_AUTH_SECRET`, and any optional ones).
