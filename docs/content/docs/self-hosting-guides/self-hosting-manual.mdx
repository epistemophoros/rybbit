...
Coolify uses Traefik as its proxy. Use the dedicated Compose file and **do not use a `.env` file** — set all configuration in Coolify’s **Environment Variables** UI only.

1. In Coolify, create a new project and add a resource using **`docker-compose.coolify.yaml`** from the repo as the compose source.
2. Leave **"Escape special characters in labels"** unchecked so `${DOMAIN_NAME}` (and other vars) in labels are resolved.
3. Set **all** required and optional variables in Coolify’s Environment Variables (e.g. `DOMAIN_NAME`, `BASE_URL`, `BETTER_AUTH_SECRET`, `PORT`, database credentials). Do not rely on a `.env` file in the repo.
